...
export const numberOfCols = 50;
export const grid = Array(numberOfRows)
	.fill(null)
...
export function findAvailablePosition(word) {
	const horizontalSize = measureWord(word, 'horizontal');
	const verticalSize = measureWord(word, 'vertical');
...
export function drawWord(word, wordPosition) {
	const { row, col, colSpan, rowSpan, fontSize } = wordPosition;
